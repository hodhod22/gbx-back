...
import { getUserIdFromToken, AuthRequest } from "../middleware/auth";
...
export const getOrCreateStripeAccount = async (
  ...
  if (!stripeAccountId) {
    ...
    console.log(
      `Created Stripe Connect account for user ${userId}: ${stripeAccountId}`,
    );
  }
...
// ============ GET MAX PAYOUT ============
export const getMaxPayout = async (
  req: Request,
  res: Response,
): Promise<Response> => {
  const userId = getUserIdFromToken(req);
  ...
  if (maxNetSek < 0) maxNetSek = 0;
  if (maxNetSek < 100) maxNetSek = 0;
...
export const requestPayout = async (
  ...
  if (!stripeAccountId) {
    stripeAccountId = await getOrCreateStripeAccount(userId, email);
  }
  ...
  const balanceRes = await pool.query(
    "SELECT balance, COALESCE(reserved, 0) as reserved FROM gbx_balances WHERE user_id = $1",
    [userId],
  );
  const balance = parseFloat(balanceRes.rows[0]?.balance || 0);
  ...
  try {
    ...
    await client.query(
      "UPDATE gbx_balances SET balance = balance - $1, reserved = reserved + $1 WHERE user_id = $2",
      [requiredGbx, userId],
    );
    ...
    if (isInstant) payoutOptions.method = "instant";
    ...
    await client.query(
      `INSERT INTO payouts (user_id, payout_id, amount_brutto_sek, amount_net_sek, gbx_amount, status, method)
       VALUES ($1, $2, $3, $4, $5, 'pending', $6)`,
      [userId, payout.id, bruttoSek, netAmountSek, requiredGbx, methodName],
    );
...
export const getOnboardingLink = async (
  ...
  const stripeAccountId = await getOrCreateStripeAccount(userId, email);
  const origin =
...
// ============ WEBHOOK HANDLER ============
export const handlePayoutWebhook = async (event: any): Promise<void> => {
  const payout = event.data.object;
  if (event.type === "payout.paid") {
    await pool.query(
      `UPDATE payouts SET status = 'completed', completed_at = NOW() WHERE payout_id = $1`,
      [payout.id],
    );
    const payoutRes = await pool.query(
      "SELECT user_id, gbx_amount FROM payouts WHERE payout_id = $1",
      [payout.id],
    );
    if (payoutRes.rows.length > 0) {
      const { user_id, gbx_amount } = payoutRes.rows[0];
      await pool.query(
        "UPDATE gbx_balances SET reserved = reserved - $1 WHERE user_id = $2",
        [gbx_amount, user_id],
      );
